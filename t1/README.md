...
### Ansible Later - enfore best practices for Role Development

Policy-driven documentation and metadata validator for Ansible Collections, Roles and Plugins.

It checks the structural and documentation quality more than task-level correctness.

**What it checks:**

- Collection and role layout
- Required metadata fields
- Plugin docstrings (DOCUMENTATION, RETURN, EXAMPLES)
- Compatibility requirements
- Galaxy import readiness
...
Validate best practices and catch common mistakes in roles, playbooks, tasks, handlers, vars, etc.
...
Ansible Creator is a CLI tool from the Ansible community that helps generate and manage Ansible project scaffolding:
collections, roles, plugins, documentation blocks, Execution Environment configs, CI templates, and more.

It standardizes project layout and allows you to quickly start new automation projects with recommended structures.

**Useful for:**

- creating new collections
- generating roles/plugins inside collections
- preparing EE build files
- integrating with IDEs (like VS Code)
- keeping consistent folder layout across teams

[Repository](https://github.com/ansible/creator)
...
#### Using Ansible Creator with VS Code

VS Code + Ansible extension works seamlessly with projects generated by Ansible Creator.

**Benefits:**

- Correct collection layout ensures the VS Code language server can resolve modules/roles/plugins.
- `galaxy.yml` and auto-generated docs help syntax completion.
- EE scaffold integrates with the VS Code Ansible extension (run playbooks using Execution Environment containers).

**Use it when you want to:**

- create consistent Ansible collections
- generate roles/plugins quickly
- standardize team project layout
- prepare Execution Environments for AWX/Automation Controller
- keep documentation blocks compliant (works with Ansible Later)
...
### Molecule - testing Ansible Roles/Collections

Molecule is a testing framework for Ansible roles.
It verifies that a role works as expected by running automated tests in isolated environments (containers, VMs, clouds - depends on driver).

It focuses on unit-like testing for roles, ensuring idempotency, correct task behavior, and reliable changes.

[Tool ref](https://molecule.readthedocs.io/)
...
The docker plugin is the most common driver. Others include podman, vagrant, delegated, etc.
...
1. Create a test scenario
    ...
    ```shell
    molecule init scenario --scenario-name default --role-name myrole
    ```
...
- create environment (e.g., Docker container)
- converge the role
- idempotence check
- verify tests (usually via pytest + testinfra)
- destroy environment

#### Example: Run a single step

In example collection created with `ansible-creator` add a new role with `ansible-galaxy init` and test it with below actions.

Remember to configure `molecule/default/molecule.yaml` to Your platform and provisioner.

Same goes for `molecule/default/converge.yaml` - here place the role to test with its task yaml.

[More here](https://docs.ansible.com/projects/molecule/getting-started-playbooks/#advanced-testing-patterns)
...
2. Apply the role
    ...
    molecule converge
    # or
    molecule test # it runs all configured tests
    ```
...
Molecule typically uses Testinfra (Python) to validate the system state.
